...
router.get("/", function (req, res) {
	...
	for (
		let i = 0;
		i < players.length && playersTodayBirthday.length < searchLimit;
		i++
	) {
...
router.get("/search", function (req, res) {
	...
	res.render("searchPage", {
		players: searchObject,
		title: "Test Heading",
		searchedWord: wordToMatch,
...
function processMatchArray(wordToMatch, searchLimit) {
	...
	if ((undefined = !wordToMatch || wordToMatch != "")) {
		...
		for (
			let i = 0;
			i < players.length && matchArray.length < searchLimit;
			i++
		) {
			...
			if (
				matchArray.length < searchLimit &&
				(playerFullName.match(regex) || playerName.match(regex))
			) {
